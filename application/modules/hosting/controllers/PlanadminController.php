 ...
 * @
 */
class Hosting_PlanadminController extends Zend_Controller_Action {
    ...
    public function addAction() {
        $form = new Hosting_Form_Plan();
        $form->setAction($this->view->url());
        $form->setMethod('post');
        if ($this->getRequest()->isPost()) {
            if ($form->isValid($_POST)) {
                $model = new Hosting_Model_Plan();
                $result = $model->create($form->getValue('name')
                                , $form->getValue('qouta')
                                , $form->getValue('bandwidth')
                                , $form->getValue('reseller')
                                , $form->getValue('cost_per_month'));
                if ($result) {
